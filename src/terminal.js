  ...
  const body = projects
    .map(
      (p, i) =>
        ...
        "</span>" +
        (p.status
          ? ' <span style="color:#e0b341">[' + p.status + "]</span>"
          : "") +
...
export function commandOutput(cmd) {
  ...
  if (base === "cat") {
    ...
    return { html: notfound("cat: " + f + ": No such file.") };
  }
  switch (base) {
    case "help":
    case "?":
    ...
    case "date":
      return {
        html:
          '<span style="color:#9fd3b0">' + new Date().toString() + "</span>",
      };
    case "sudo":
      return {
        html: '<span style="color:#e8604a">jazib is not in the sudoers file. This incident will be reported.</span>',
      };
    case "hire":
      return {
        html: '<span style="color:#74ffa6">[+] excellent choice. → jazibmalik731@gmail.com</span>',
      };
    case "":
      return { html: "" };
    default:
...
// DEVELOPED BY NITHIL KRISHNARAJ. 2026 ALL RIGHTS RESERVED.
